const defaultItems = [{name:'Lujanes',hero:'A pedido',price:350,type:'A pedido',stock:null,image:''}];
...
function render(){
 ...
 const list=catalogItems.filter(x=>(filter==='all'||x.type===filter)&&(!q||`${x.name} ${x.hero||''} ${x.type}`.toLowerCase().includes(q)));
 catalog.innerHTML=list.map(x=>{
   ...
   return `<article class="card ${available?'':'soldout'}" tabindex="0" role="button" aria-label="Consultar ${esc(x.name)}" data-wa="${wa}">
     ...
     <div class="card-body"><h3>${esc(x.name)}</h3><div class="hero-name">${esc(x.hero||'Dota 2')} · ${esc(x.type)}</div><div class="price">S/ ${esc(x.price)}</div><div class="stock">${esc(stockText)}</div>${duration?`<div class="duration">${duration}</div>`:''}
